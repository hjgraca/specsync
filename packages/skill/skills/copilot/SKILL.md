...
Use the shell tool to create a Q&A session. For each question, include your recommended answer with reasoning.
...
1. Parse the response JSON to get the session `id` and `token`
2. Tell the user: "Q&A session ready at {url from response} — answer in the browser."
...
5. Read the `answers` object from the final response and continue your work using those answers.
...
5. Continue once a decision event is present in the polled response.
...
- Prefer polling with the shell tool over asking the user to return to the CLI
