...
If the server is not running, tell the user: "Please start the specsync server with: npx specsync start"

## When the user wants to ask the team questions

Use the terminal tool to create a Q&A session. For each question, include your recommended answer with reasoning.
...
Use the terminal tool to publish the spec:
...
curl -s -X POST "${REVIEW_TOOL_URL:-http://localhost:4000}/documents/{slug}/ops" \
  ...
  -H "Content-Type: application/json" \
  -d @- << 'EOF'
{"type": "comment.reply", "markId": "MARK_ID", "by": "ai:cursor", "text": "Your reply"}
EOF
```
...
- Generate a unique codename for yourself: `ai:cursor-<adjective>-<noun>` (e.g., `ai:cursor-quiet-mountain`). Use a random pair. Use this in ALL `by` fields.
