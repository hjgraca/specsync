...
curl -s -X POST "${REVIEW_TOOL_URL:-http://localhost:4000}/documents/{slug}/ops" \
  ...
  -H "Content-Type: application/json" \
  -d @- << 'EOF'
{"type": "comment.reply", "markId": "MARK_ID", "by": "ai:kiro", "text": "Your reply"}
EOF
```
...
- Generate a unique codename for yourself: `ai:kiro-<adjective>-<noun>` (e.g., `ai:kiro-swift-falcon`). Use a random pair. Use this in ALL `by` fields.
